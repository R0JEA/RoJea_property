...
$('document').ready(function () {
  ...
  document.onkeyup = function (data) {
    if (data.which == 116) {
      $.post('https://RoJea_property/toggleCursor');
    }

    if (data.which == 13) {
      if (selectedObjectData != null && $(".decorate-confirm").css('display') != 'block') {
        $.post('https://RoJea_property/editOwnedObject', JSON.stringify({
          objectData: selectedObjectData
        }));
        selectedObjectData = null;
        $.post('https://RoJea_property/setupMyObjects', JSON.stringify({}), function (myObjects) {
          $('.decorate-items').html("");
          $.each(myObjects, function (i, object) {
            var elem = '<div class="decorate-item" id="myobject-' + i + '" data-type="myObject"><span id="decorate-item-name"><b>Objektas: </b>' + object.hashname + '</span><span id="decorate-item-category"><strong>Kaina: </strong><span id="item-price" style="color: green;">NUPIRKTAS</span></span></div>';
        ...
        if (objData != null) {
          $.post("https://RoJea_property/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function () {
  ...
  $.post('https://RoJea_property/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function () {
  ...
  if (selectedCategory != "remove-owned-obj") {
    ...
    $.each(houseCategorys[selectedCategory].items, function (i, item) {
      var elem = '<div class="decorate-item" id="object-' + i + '" data-type="newObject"><span id="decorate-item-name"><b>Objektas: </b>' + (item.label).charAt(0).toUpperCase() + '' + (item.label).substr(1).toLowerCase() + '</span><span id="decorate-item-category"><strong>Kaina: </strong><span id="item-price" style="color: green;">€' + item.price + '</span></span></div>';
      $('.decorate-items').append(elem);
    ...
    $(".decorate-footer-buttons").html("");
    $(selectedHeaderButton).removeClass('header-btn-selected');
    $.post('https://RoJea_property/deleteSelectedObject');
    $(".decorate-footer-buttons").fadeOut(150);
    $(".decorate-items").fadeOut(150);
  }
});
...
$(document).on('click', '#buy-object', function () {
  $.post("https://RoJea_property/buySelectedObject", JSON.stringify({
...
$(document).on('click', '#cancel-object', function () {
  $.post('https://RoJea_property/cancelSelectedObject');
  selectedObjectData = null;
  $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function () {
  ...
  if ($("#" + objId).data('type') == "newObject") {
    if (selectedObject == this) {
      ...
      selectedObject = null;
      $.post('https://RoJea_property/removeObject');
    } else {
      $(this).addClass('selected-object');
      selectedObject = this;
      $.post("https://RoJea_property/chooseobject", JSON.stringify({
        object: objData.object,
      }));
    ...
    if (selectedObject == this) {
      $(this).removeClass('selected-object');
      $.post('https://RoJea_property/deselectOwnedObject')
      selectedObject = null;
    } else {
      $(this).addClass('selected-object');
      selectedObject = this;
      selectedObjectData = myObjectData;
      $.post('https://RoJea_property/selectOwnedObject', JSON.stringify({
        objectData: myObjectData
      }))
      $(".decorate-footer-buttons").html("");
      var elem = '<div class="footer-btn" id="remove-owned-obj"><p>Trinti objekta</p></div>';
      $(".decorate-footer-buttons").append(elem);
      $(".decorate-footer-buttons").fadeIn(150);
    }
  }
...
Decorations.Close = function () {
  ...
  selectedObjectData = null;
  $.post("https://RoJea_property/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function (e) {
  $.post('https://RoJea_property/buy', JSON.stringify({}))
});

$(".property-cancel").click(function (e) {
  $.post('https://RoJea_property/exit', JSON.stringify({}));
});
